...
import { Matcher, UpcastWriter } from '@ckeditor/ckeditor5-engine';
...
export function transformListItemLikeElementsIntoLists(documentFragment, stylesString) {
	...
	for (const [i, itemLikeElement] of itemLikeElements.entries()) {
		...
		if (!currentList || indentationDifference !== 0) {
			...
			if (!currentList) {
				...
				currentIndentation = Number.parseInt(itemLikeElement.indent);
			}

			if (
				itemLikeElement.indent <= currentIndentation &&
				!currentList.is('element', listStyle.type)
			) {
...
function detectListStyle(listLikeItem, stylesString) {
	...
	const listStartIndexRegex = /mso-level-start-at:\s{0,100}(\d{0,10})\s{0,100};/gi;
	...
	if (listStyleMatch && listStyleMatch[1]) {
		...
		if (listStyleType === 'bullet') {
			...
			if (listStartIndexMatch && listStartIndexMatch[1]) {
				startIndex = Number.parseInt(listStartIndexMatch[1]);
			}
		}
	}
