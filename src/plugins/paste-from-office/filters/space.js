...
export function normalizeSpacerunSpans(htmlDocument) {
	for (const el of htmlDocument.querySelectorAll('span[style*=spacerun]')) {
		...
		if (el.innerText.trim().length === 0) {
			...
			el.innerHTML = Array.from({ length: innerTextLength + 1 })
				.join('\u00A0 ')
				.slice(0, Math.max(0, innerTextLength));
		}
	}
}
	...
	return htmlString.replace(
		...
		(fullMatch, spaces) => {
			return spaces.length === 1
				? ' '
				: Array.from({ length: spaces.length + 1 })
						.join('\u00A0 ')
						.slice(0, spaces.length);
		}
	);
}
